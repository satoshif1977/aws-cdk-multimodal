...
同ディレクトリの retry.py と同じ「AWS 呼び出しの運用品質を揃える」方針の
ユーティリティで、retry_logger() を retry_call の on_retry に渡して結線できる。

process_doc Lambda は CDK の Code.fromAsset('lambda_src/process_doc') で
ディレクトリごと zip 化されるため、logger.py は lambda_function.py と
同一ディレクトリに配置される（実行時も同じ階層）。
